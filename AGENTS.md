...
Frame every task as **GOAL / CONTEXT / CONSTRAINTS / DONE_WHEN** (specifics in the prompt; long-lived rules in the repo). **Fewest round-trips:** a model round-trip is the expensive unit; a tool result inside a turn is cheap. Turn a task into one runtime-managed command graph, dispatching independent reads, searches, and calls together so deterministic execution continues without another model round-trip. Two forms earn it: dispatch independent reads, searches, and calls together in one turn; and collapse a deterministic multi-step sequence into one batched command (a shell pipeline or a slash command) instead of walking it call by call. Then:

- **THINK (discover/orchestrator):** classify (§2), define DONE_WHEN, run the fit gate (§2), gather primary-source evidence in parallel, commit to exactly one recommendation, and stop gathering the moment more evidence cannot change the next action.
