...
- **Follow the comment rule** (`effective-code-craft`): default to *no* comment; add one only when the code cannot convey the *why* -- a non-obvious constraint, invariant, external contract, or historical gotcha. Keep it to one terse line on the *why*; if you need more, the code likely needs a name, not a paragraph. Prefer fixing clarity over annotating it.
