...
- **Follow the comment rule** (`effective-code-craft`): the default is *no* comment. Clear all three gates before writing one -- naming is exhausted, it states *why* not *what*, and the *why* is not derivable from the code. When warranted, exported / public symbols take a doc comment in the language's official convention (godoc, TSDoc/JSDoc, rustdoc, docstring, Javadoc); private code takes at most one terse *why* line. Never annotate the obvious, never emit banner dividers or tracker-duplicating `// TODO`s. Prefer fixing clarity over annotating it.
