...
4. **Execute** — small atomic commits, build green at every step, public behavior frozen. Apply [craft](../skills/craft/SKILL.md); apply [performance](../skills/performance/SKILL.md) only after correctness holds and only on measured hot paths. Restructuring is what falsifies comments first: delete the ones that restate the code rather than porting them to the new shape.
