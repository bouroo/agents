...
- **Comments:** default to none (rule owned by effective-code-craft "Clarity"). Public APIs keep idiomatic doc comments (godoc, JSDoc, rustdoc); otherwise comment only the *why*. Flag as SHOULD FIX any comment that restates code, narrates an obvious step, over-annotates every branch, or cites internal harness references -- plan/task IDs (`U1`, `T16`), decision IDs (`D5`, `D8`), spec line numbers (`spec §3`), handoff paths, or tracking tokens (`PENDING;`). Those belong in `.agents/` artifacts, not source; if the point is a durable constraint, state it standalone.
