...
- **`PROMPT:` at intake, before the first edit or command:** `PROMPT: <one concise English paragraph — GOAL / CONTEXT / CONSTRAINTS / DONE_WHEN>`. Pin the ask in English before working, so the reading is on the record while it can still be corrected. Write it from what the user said, not from what you intend to do; a restatement you cannot write without guessing is the trigger to stop and ask one pointed question with your recommended interpretation, and the run does not proceed until the reading is confirmed.
