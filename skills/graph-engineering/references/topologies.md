...
> Load on demand from [graph-engineering](../SKILL.md): the canonical topology, the edge-condition patterns, and the cost arithmetic behind the keep/drop verdict. Structure is doctrine; numbers are the guide's claims — re-derive them on your own runs before citing any.

## The canonical topology

The guide's reference shape — five nodes, every edge conditioned:
...
The review lenses fan out concurrently and rejoin at the synthesizer; the gate's fail edge carries the synthesized findings back to the worker. That fail edge is the whole feedback loop, so its payload (findings, not a bare "failed") and its cap are design decisions, not afterthoughts.
...
| --- | --- | --- |
| Reviewer fails, blocking | back to the Worker with the findings | the worker needs the delta, not just the flag |
| Reviewer fails, non-blocking | flag to the Synthesizer | the verdict carries the caveat forward |
| Gate fails after rework | step down to a staged loop | per the matrix: the concurrency did not pay |
...
- A sequential loop passing with per-cycle probability p spends, in expectation, 1/p cycles.
...
Auto-derived graphs decay: the guide's illustration prices 85% per-hop entity resolution at ~44% trustworthiness over five hops (0.85^5). Human-authored edges — this skill's target — sidestep the compounding, but not expiry: stamp each edge with creation and last-verification dates, and treat an undated edge as stale until proven current.

## Cross-references

- [graph-engineering](../SKILL.md) the method this file works out.
- [verification](../../verification/SKILL.md) the evidence standard at the gate; the loop caps that bound every route back.
