...
Durable operating doctrine for the `mcp-atlassian` server against `TENANT.atlassian.net` (space `ClientInstance`). Captures every trap hit in the <example-service> wiki work so it is never re-derived. Detail lives in `./references/`; load only what the task needs.
...
- [references/page-template.md](./references/page-template.md) + `page_template.py` -- the exact endpoint-page template extracted from the hand-built cart/get example (`<tiny-link>`): section order, fixed table column sets, highlighted metadata layout, wide-breakout code macros. Load + run the generator when a new page must match that example.
