...
- [page-template.md](references/page-template.md) + [page_template.py](page_template.py) the endpoint-page template generator (section order, fixed table column sets, highlighted metadata, code macros). Load + run when a new page must match that example. It also carries **mandatory content-quality rules** (every nested struct field as its own dotted-path row; full-field samples with internally consistent mock data; json-tag-verified field names) and an **instance-variant section** (ktbinnovation "BFF API Specification" pages, incl. the plain-code-block sequence-diagram form); match siblings when extending an existing page family.
