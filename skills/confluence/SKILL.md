...
- **Never copy a collapsed macro signature back as markdown** (paid for 2026-09-23: three sample blocks corrupted this way). Page reads render code/diagram macros as one signature line — `wide4000json{`, `textwide4000true@startuml`, `bottomname.svg<base64>` — that is the RENDERED form, not source. When re-authoring a section, re-fence every sample (` ```lang ` fences — the converter rebuilds the macro); an unfenced signature line followed by newline-JSON merges into a single text paragraph on conversion. Escaping differs by surface: table cells take a single `\_` / `\*` (what the original authoring stored — renders clean), code-fence bodies take plain characters, and a doubled `\\_` renders a visible backslash — never double-escape.
- **Section updates are boundary-blind past the headings the matcher recognizes.** `update_page_section` replaces from heading_text to the next heading the tool can see; on pages whose deeper headings (h5/h6, authoring-dependent) it cannot match, the replacement swallows EVERYTHING to EOF (paid for 2026-09-23: two spec pages lost all sections after their change log). On an unfamiliar page, attempt the real section edit first — a heading miss is a clean error that writes nothing — and always run `get_page_diff` (n-1)→n after a section write; restore from the diff's removed lines or `get_page_history(version=N, convert_to_markdown=false)` storage when boundaries were eaten.
- **Diagrams default to PlantUML** through the instance's diagram macro; a bare `<pre><code>@startuml...` NEVER renders as a diagram (it shows literal text - the one failure mode invisible when re-reading the body). On Rovo, mirror the sibling's Confluence-HTML macro node; on mcp-atlassian, author the storage-format macro directly ([references/page-template.md](references/page-template.md) `[storage-form]` snippets apply verbatim). Encode the `data` param with the **gated encoder** in that reference (quote source `safe="/"` → raw deflate 6 → base64; the param must be pure base64, no `%`) and get its `GATE OK` line **before** any publish - a blank diagram is invisible in the stored body, so the gate is the only pre-push defense. Every published diagram ships with its raw source mirrored in a collapsed expand so it survives macro outages.
...
- **Update-change visibility (highlight + changelog):** a page *update* must make what changed visible in the page itself, not only in version history. Before rewriting, fetch the pre-change baseline — mcp-atlassian `confluence_get_page_history(page_id, version=N, convert_to_markdown=false)` returns the full stored body of version N (Rovo: `getConfluencePageDiff` across versions) — walking N = current-1, current-2, … until the version's timestamp precedes the current effort, and diff section/field sets baseline vs new body. Then remark every added/changed point with refs that resolve in-page: rebuilt tables carry a `Change` column whose New/Modified/Renamed cells hold `C#` (bold the changed field name), removed content ships as `Removed` rows in a `Change Log` table (h3 near the changed sections: `Change # | Date | Field/Section | Change Type | Description`, Date = edit date, per-page `C1..Cn`), and prose-level semantic shifts take a page-level row (Field `—`). Prove the refs bidirectional after publish: every `C#` in the body exists in the Change Log and vice versa, under documented relaxations (`—` page-level rows, `X[].*` aggregate-prefix rows, `Removed` rows anchored only in the baseline). Snippets and the integrity-gate wording: [references/page-template.md](references/page-template.md) `[changelog]`.
- **Large pages / payload corruption:** big inline bodies fail in two distinct ways - gateways reject outright (502s/reset; retry after ~20 s), but the MCP client→server hop can also **silently corrupt** the call before validation: the tool rejects with `InputValidationError ... could not be parsed as JSON` and reports a byte count ~3× the payload you sent (duplicated/truncated JSON). That is a transport defect, not an authoring mistake - do not reformat the content; change the transport. Emit MCP writes **sequentially, one call per message** - parallel write calls in one block fused into a single garbled ~100 KB JSON once (2026-09-23, one key repeated thousands of times; rejected pre-write, but only after two dead attempts). Solo writes of 15-19 KB inline markdown went through fine after that - size was never the trigger, batching was. Ladder (all proven 2026-09-07): smaller `confluence_update_page_section` (~2.4 KB went through where ~4-16 KB corrupted) → full-body `confluence_update_page` with `content_file` (the tool reads the body from disk - immune to inline corruption, worked at 12-16 KB). `content_file` paths are sandboxed to the server's working directory - a `/tmp` payload is rejected as path traversal; stage payloads inside the workspace (e.g. `.agents/<task>/`) and delete them after.
- **Diagrams from Mermaid sources:** mermaid fences never publish - translate each diagram to the family's PlantUML form ([references/page-template.md](references/page-template.md) has the element mapping and the `plantumlcloud` `data` encoding recipe). Keep the raw Mermaid in the repo doc; only the translated PlantUML ships.
- **When body reads collapse to pointers (`<<ccr:…>>`):** the read-side twin of the prove-after-write problem, and the loop hazard. The ref is a cached *deterministic* result per (page, format) — identical `get_page` retries return the identical ref, so refetching the same page more than once per variable change is a loop, not persistence (cap: two identical retries). Read fallbacks, strongest first (proven 2026-09-11): `confluence_get_page_diff` between **adjacent recent versions** — intermittently returns the full unified diff, and after your own section edit it doubles as a **recovery oracle**: the diff shows exactly what was replaced, so a bad section edit can be restored verbatim from the removed lines; `confluence_search` with `siteSearch ~ "<distinctive phrase>"` **scoped by `title = "<page>"`** — excerpts are match-centered but *blend fragments across sections and pages*, so attribute text to a page only via a title-scoped probe, never from an unscoped hit; and remember CQL `ancestor = X` matches **descendants only** — the parent itself needs `title =`. **Write discipline while reads are down:** replace only *schema-bounded* sections (index catalogs, lineage tables, anything fully recomputable from code/DDL — their failure mode is a clean heading-miss error, and the post-edit diff exposes any clobbered content for restore); never append to or rewrite changelogs and field dictionaries (unseen rows would be destroyed); never hand-reconstruct a full page body from excerpts, memory, or inference — that is fabrication, not sync, and it silently destroys the unseen majority of the page. Park every remaining stale cell in a dated footer comment (`confluence_add_comment`) as the fix checklist, and PENDING it in the report.

**Publish-then-prove:** after any write, re-fetch the page and confirm the stored body contains what you intended (macro wrappers present, sources escaped): Rovo `getConfluencePage(contentFormat="html")`, mcp-atlassian `confluence_get_page(convert_to_markdown: false)`. A narrated success without the read-back is theater - and so is a read-back the transport collapses: mcp-atlassian can return `<<ccr:…>>` refs instead of bodies; that proves nothing either way, route to the probes below. Front-load what no probe can catch post-push: diagram encoding defects are invisible in the stored body (blank render, valid-looking XML), so the pre-push `GATE OK` ([references/page-template.md](references/page-template.md)) is the only defense - run it, don't claim it. When the read-back comes back truncated, collapsed, or summarized, fall back to probes in descending strength (proven 2026-09-07): `confluence_get_page_diff` between the previous and new version (a non-empty diff plus the bumped `version` in the write response is the server-side witness, even when the diff body itself collapses); `confluence_search` with `text ~ "<string unique to the new body>"` (search indexes stored body text - fresh pages can lag minutes in the index, and macro **parameters** are not indexed at all, so a miss proves nothing about a `data` param); the update response's bumped `version` number; the parent's children listing. The strongest local proof for a body you pushed via `content_file`: assert distinctive strings (new data param present, old one absent) against the pushed file - but remember that proves what you *sent*, not what the server stored; keep the version bump as the server-side witness. Whether a diagram actually *paints* is only visible in a browser - state that limit honestly instead of claiming render success, and when fixing a blank diagram never trust another page's encoding as "known-good" unless someone has seen that page render (the incident is recorded in [references/page-template.md](references/page-template.md)).
