...
Durable operating doctrine for the `mcp-atlassian` server against `ktbinnovation.atlassian.net` (space `GoofyDisco`). Captures every trap hit in the adapter-mbf-ais wiki work so it is never re-derived. Detail lives in `./references/`; load only what the task needs.
