...
- **Large pages:** create with a minimal placeholder body plus real title/parent, then push the full body via a second update call (`content_file` accepts a filesystem path on mcp-atlassian). Gateway transport errors (502s/reset) hit big payloads; retry after ~20 s. `content_file` paths are sandboxed to the server's working directory - a `/tmp` payload is rejected as path traversal; stage payloads inside the workspace (e.g. `.agents/<task>/`).
