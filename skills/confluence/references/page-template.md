...
an instance-variant record below (the 2026-09-09 storage-form anchor
`wiki/x/AYBUdAE`) so the next run starts from evidence, not re-derivation.
...
### Storage-form record (mcp-atlassian surface): `wiki/x/AYBUdAE`

Decoded 2026-09-09 from `bff-mobile-mbf - POST /v1/applications/installmentation #3`
(space `GoofyDisco`, v5) - the storage-format rendering of this family. Same
skeleton as the Rovo record, with these load-bearing divergences; when
publishing through mcp-atlassian, match this record:
