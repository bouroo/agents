...
Code stopped being the bottleneck once agents could write it faster than humans could plan, review, and ship around it. The constraint moved outward — to intent, review, and governance — so the job is a **loop**, not a phase sequence: each stage commits a readable artifact the next begins from, and that artifact chain *is* the audit trail. Seven stages; `Operate` reopens `Intent`.
...
A stage exits only on its check. "The agent finished" is not an exit; the artifact and its check are. [artifacts](../artifacts/SKILL.md) owns the shapes; this skill owns the stage map.

## It is a loop, not a waterfall

Phase-gated delivery answered a bottleneck (hand-written code) that no longer exists; keeping the gates after removing the bottleneck leaves planning, review, and release at human speed while build collapses to hours. So stages may be **re-entered**, with two canonical back-edges:

- **`Test -> Build`** — a refuted verdict returns to the implementer with findings, not a flag; the ordinary review cycle, bounded by the 3-failed-cycles rule ([verification](../verification/SKILL.md)).
...
Seven seats, not seven people — one actor may hold several, and small jobs compress them:
...
Two constraints never bend, however the seats collapse: **the Implementer never approves its own work**, and **the Verifier is independent of what it judges** — a different context, not the author's memory of writing it. Org roles collapse into these seats rather than adding more: release manager is the Approver, auditor a Verifier, on-call the Operator, product or policy owner a Steward. Adding a seat that duplicates one is the enterprise-ceremony failure this cast exists to prevent.

## Human judgment concentrates at the seams

Implementation is the agent's; approval is not. Every judgment-bearing artifact — accepted intent, agreed spec, authorized release — stays human-accountable, and the audit trail is those artifacts in the repository, not a narration about them. Policies are applied *while* artifacts are produced, not discovered in a later review; a policy that must always hold is backed by a deterministic gate rather than an instruction to remember it. Automation stops at the production gate — approval happens above that boundary.

## Where verification and evals attach

Every stage's exit is a verification event — the three layers (L1/L2/L3), the evidence audit, and the mutation probe live in [verification](../verification/SKILL.md). Distinct from that, the lifecycle is itself configuration an agent executes, so it is regression-tested like production code: the [evals](../evals/SKILL.md) suite runs realistic tasks against the manifesto and skills and blocks a change that lowers the pass rate. Evals test *the process*; verification tests *a unit of work*.

## Within a stage, the execution graph

The lifecycle says *which stage and what it commits*; how a stage's work is shaped inside is the execution graph — nodes, typed edges, caps, shapes, and the minimal-harness ladder that enters at the least agency closing on evidence ([graph-engineering](../graph-engineering/SKILL.md)). A stage contains one or more graphs; the two compose, neither replaces the other.
