...
4. **Safe by default.** Make invalid states unrepresentable; prefer validating constructors that refuse bad input at construction time over types callers can assemble wrongly; named constants over magic values; least privilege; encode rules in types/validators not model judgment; explicit error returns, never in-band sentinels.
