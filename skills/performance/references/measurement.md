...
2. **Write an atomic benchmark**: isolate one function per benchmark to avoid contamination. A benchmark touching everything proves nothing about anything.
...
- An **off-CPU profiler** shows I/O wait. If off-CPU dominates, the bottleneck is external.
...
- **Isolate one function per benchmark.** One touching everything produces contaminated, unattributable results.
