...
> Load on demand. The short cycle lives in [performance-patterns](../SKILL.md) §Measure Before You Optimize; this file is the detail.

Optimization without measurement is guessing, and guesses about bottlenecks are wrong ~80% of the time. Every step below produces an artifact: a captured command, a report file, a benchmark result. The artifact is the evidence; the narrative is not.
...
- **Beware micro-benchmark traps.** Dead-code elimination, constant folding, and escape analysis can make a benchmark "prove" a speedup that the real code never gets. Inspect the generated assembly or IR when a result looks too good.
